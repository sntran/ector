defmodule Ector.RepoTest do
  ...
  test "all/one/delete_all/update_all pass through standard Ecto queryables" do
    ...
    assert [%PlainWidget{id: "widget-1"}, %PlainWidget{id: "widget-2"}, %PlainWidget{id: "widget-3"}] =
    ...
    assert {:ok, %PlainWidget{id: "widget-1", name: "Renamed"}} =
             repo.delete(Ecto.Changeset.change(repo.one(from(widget in PlainWidget, where: widget.id == "widget-1"))))
  ...
  test "repo helpers preserve defaults and fall back on unsupported delete and update shapes" do
    ...
    assert [
             %User{id: "user-atom", name: "Atom", metadata: %{flags: [true, false]}},
             %User{id: "user-default", name: nil, metadata: %{}}
           ] =
  ...
  test "insert/2 rejects non-Ector nested targets" do
    ...
    source_changeset =
      Ector.Changeset.put_edge(source_changeset, :carts, [{target_changeset, %{"status" => "active"}}])
  ...
  test "insert/4 surfaces storage insert failures" do
    ...
    assert {:error, invalid_changeset} =
             Ector.Repo.insert(RejectingInsertRepo, changeset, [], fn _struct, _opts -> flunk("unexpected fallback") end)

    assert {"expected a single storage row to be inserted", _opts} = invalid_changeset.errors[:base]
  end
  ...
  test "insert/4 surfaces edge insert failures" do
    ...
    assert {:error, invalid_changeset} =
             Ector.Repo.insert(RejectingEdgeInsertRepo, source_changeset, [], fn _struct, _opts -> flunk("unexpected fallback") end)
  ...
  test "insert/4 uses the non-SQLite storage insert path for Postgres adapters" do
    ...
    assert {:ok, %User{id: "user-postgres"}} =
             Ector.Repo.insert(SuccessfulPostgresInsertRepo, changeset, [], fn _struct, _opts -> flunk("unexpected fallback") end)
  end
  ...
  test "insert/2 supports module through labels, binary through labels, named associations, and rejects invalid edge routing metadata" do
    ...
    module_through = UserWithModuleThrough.changeset(%UserWithModuleThrough{}, %{"id" => "module-user", "name" => "Ada"})
    ...
    binary_through = UserWithBinaryThrough.changeset(%UserWithBinaryThrough{}, %{"id" => "binary-user", "name" => "Ada"})
    ...
    named_edge = UserWithNamedEdge.changeset(%UserWithNamedEdge{}, %{"id" => "named-user", "name" => "Ada"})
    ...
    invalid_edge = UserWithInvalidThrough.changeset(%UserWithInvalidThrough{}, %{"id" => "bad-user", "name" => "Ada"})
